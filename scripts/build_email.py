...
Reads the most recent ACR snapshot, filters issuance and retirement records by date
window (last Friday + 1 day → this Friday), classifies IFM vs Non-IFM by OHA's
category rules, generates four PNG charts, and renders the email HTML from a
Jinja2 template.

Filter logic:
  - Issuance:    credits with status Active or Retired AND issuance date 'd' in window
  - Retirements: retired records with retirement date 'd' in window

(Earlier versions of this script diffed snapshots by serial number; that approach
broke because ACR re-issues serial numbers when credits change sub-status. The 'd'
date field is the canonical source of truth for when a credit was issued/retired.)
...
def filter_week_activity(this_wk: dict, last_friday: date, this_friday: date) -> tuple[list, list]:
    """Return (issuances_this_week, retirements_this_week).

    'This week' = the calendar window (last_friday, this_friday] — i.e., from the day
    AFTER last Friday through this Friday inclusive.

    Issuances: credits with status in ISSUANCE_STATUSES whose 'd' (issuance date)
    falls in the window.
    Retirements: retired records whose 'd' (retirement date) falls in the window.

    Note: we deliberately use this_wk (current snapshot) only — not the last_wk diff —
    because ACR re-issues serial numbers when credits change sub-status, which makes
    serial-number diffing unreliable. The 'd' date is the canonical date field.
    """
    # Window starts day AFTER last Friday (so that last Friday's data isn't double-counted
    # if the email runs both Fridays). Window ends on this Friday (inclusive).
    from datetime import timedelta
    window_start = (last_friday + timedelta(days=1)).isoformat()
    window_end = this_friday.isoformat()

    issuances = [
        c for c in this_wk['credits']
        if c.get('s') in ISSUANCE_STATUSES
        and window_start <= (c.get('d') or '') <= window_end
    ]

    retirements = [
        r for r in this_wk['retired']
        if window_start <= (r.get('d') or '') <= window_end
    ]

    return issuances, retirements
...
def main():
    ...
    # Compute this week's activity using date window (last_date, this_date]
    new_credits, new_retired = filter_week_activity(this_wk, last_date, this_date)
    print(f'This week: {len(new_credits)} issuances, {len(new_retired)} retirements')
